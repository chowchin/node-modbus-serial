 ...
 * @constructor
 */
var NetSocketPort = function(socket, ip, options) {
    var modbus = this;
    this.ip = ip;
    this.openFlag = false;
    this.callback = null;
    ...
    this._client.on("data", function(data) {
        ...
        // data recived
        modbusSerialDebug({ action: "receive net socket strings", data: data });
        ...
        while (data.length > MIN_MBAP_LENGTH) {
            ...
            // debug
            modbusSerialDebug({ action: "parsed net socket", buffer: buffer, transactionId: modbus._transactionIdRead });
    ...
    this._client.on("connect", function() {
        ...
        modbusSerialDebug("Net Socket: signal connect");
        handleCallback();
    });

    this._client.on("close", function(had_error) {
        modbus.openFlag = false;
        modbusSerialDebug("Net Socket: signal close: " + had_error);
        handleCallback(had_error);
    });

    this._client.on("error", function(had_error) {
        modbus.openFlag = false;
        modbusSerialDebug("Net Socket: signal error: " + had_error);
        handleCallback(had_error);
    });

    this._client.on("timeout", function() {
        modbus.openFlag = false;
        modbusSerialDebug("Net Socket: TimedOut");
...
util.inherits(NetSocketPort, EventEmitter);
 ...
 * @param callback
 */
NetSocketPort.prototype.open = function(callback) {
 ...
 * @param callback
 */
NetSocketPort.prototype.close = function(callback) {
    this.callback = callback;
    this._client.end();
};
 ...
 * @param callback
 */
NetSocketPort.prototype.destroy = function(callback) {
    this.callback = callback;
 ...
 * @param data
 */
NetSocketPort.prototype.write = function(data) {
...
/**
 * Net Socket port for Modbus.
 *
 * @type {NetSocketPort}
 */
module.exports = NetSocketPort;
